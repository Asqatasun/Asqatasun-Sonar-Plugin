...
import org.sonar.plugins.asqatasun.rules.AsqatasunCheckClasses;
...
public final class AsqatasunSensor implements Sensor {
  
private static final Number[] FILES_DISTRIB_BOTTOM_LIMITS = {0, 5, 10, 20, 30, 60, 90};
  private static final Logger LOG = LoggerFactory.getLogger(AsqatasunSensor.class);

  private final Asqatasun asqatasun;
  private final NoSonarFilter noSonarFilter;
  private final AnnotationCheckFactory annotationCheckFactory;
  private final ModuleFileSystem fileSystem;
  private final FileLinesContextFactory fileLinesContextFactory;

  public AsqatasunSensor(
          Asqatasun asqatasun,
          RulesProfile profile, 
          NoSonarFilter noSonarFilter, 
          ModuleFileSystem fileSystem, 
          FileLinesContextFactory fileLinesContextFactory) {
    this.asqatasun = asqatasun;
    this.noSonarFilter = noSonarFilter;
    this.annotationCheckFactory = AnnotationCheckFactory.create(profile, AsqatasunRulesRepository.REPOSITORY_KEY, AsqatasunCheckClasses.getCheckClasses());
  ...
  public void analyse(Project project, SensorContext sensorContext) {
    ...
    for (java.io.File file : fileSystem.files(FileQuery.onSource().onLanguage(AsqatasunConstants.LANGUAGE_KEY))) {
  ...
  public boolean shouldExecuteOnProject(Project project) {
    return AsqatasunConstants.LANGUAGE_KEY.equals(project.getLanguageKey()) ||
      (StringUtils.isBlank(project.getLanguageKey()) &&
      !fileSystem.files(FileQuery.onSource().onLanguage(AsqatasunConstants.LANGUAGE_KEY)).isEmpty());
  }
